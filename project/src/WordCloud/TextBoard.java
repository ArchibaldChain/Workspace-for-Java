...
class WritePanel extends JPanel {
	...
	static Image img = new ImageIcon("Image/HeartBackGround.jpg").getImage();
	private static ArrayList<Scribbling> text = new ArrayList<Scribbling>();

	static private void doWriting(Graphics g) {
		...
		for (Scribbling s : text) {
			...
			TextLayout layout = new TextLayout(s.what, font, g2d.getFontRenderContext());
			if (s.bgCol != null) {
				Rectangle2D bounds = layout.getBounds();
				bounds.setRect(bounds.getX() + s.xpos, bounds.getY() + s.ypos, bounds.getWidth(), bounds.getHeight());
	...
	public void addText(String what, int xpos, int ypos, double angle, Font font, Color col, Color bgCol) {
	...
	void clear() {
        text = new ArrayList<Scribbling>();
	}
}

/**
 * Textboard is in fact a full graphical application that displays text in a
 * graphical Window
 */
public class TextBoard extends JFrame {
	...
	public TextBoard(int width, int height, Color col) {
		...
		// When users close the window, we save the graph

		addWindowListener(new WindowAdapter() {
			public void windowClosing(WindowEvent e) {
                int a = doCloseAction(e);
                if (a == 1) {
	...
	private int doCloseAction(WindowEvent arg){
	    return Save.saveJpg(this);
    }
	 ...
	 * @param x
	 *            Horizontal position in pixels, from the left hand side
	 * @param y
	 ...
	 * <p>
	 * The text can be written at any position, it can be rotated, the font and
	 * color can be changed. Every piece of text is added to a list, and only
	 * written when the window is drawn.
	 *
	 * @param what
	 *            The text to write
	 * @param xpos
	...
	public void write(String what, int xpos, int ypos, double angle, Font font, Color col, Color bgCol) {
	...
	public void write(String what, int xpos, int ypos, double angle, Font f, Color c) {
		write(what, xpos, ypos, angle, f, c, null);
	}

	public void write(String what, int xpos, int ypos, Font f, Color c, Color bg) {
	 ...
	 * Write text to the screen
	 * <p>
	 * Use the last set angle (0 by default), font and color (black by default).
	 *
	 * @param what
	 *            The text to write
	 * @param xpos
	 ...
	 *            Text color
	 */
	public void write(String what, int xpos, int ypos, double angle, Color c) {
	 ...
	 *            Text color
	 */
	public void write(String what, int xpos, int ypos, Font f, Color c) {
